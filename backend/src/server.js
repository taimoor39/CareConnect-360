...
import { DEFAULT_ENGAGEMENT_TEMPLATES, emptyTemplatePatch } from './utils/engagementTemplates.js';
...
/** Ensures SystemSettings singleton exists and empty template bodies are filled. */
  ...
  let settings = await SystemSettings.findOneAndUpdate(
    {},
    {
      $setOnInsert: {
        ...
        emailTemplates: DEFAULT_ENGAGEMENT_TEMPLATES,
      },
    },
    { upsert: true, new: true, setDefaultsOnInsert: true },
  );

  const patch = emptyTemplatePatch(settings.toObject ? settings.toObject() : settings);
  if (Object.keys(patch).length) {
    settings = await SystemSettings.findOneAndUpdate({}, { $set: patch }, { new: true });
    invalidateSettingsCache();
    console.log('[STARTUP] Filled empty engagement email templates:', Object.keys(patch).join(', '));
  }
