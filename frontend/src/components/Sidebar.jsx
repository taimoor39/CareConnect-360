    ...
    <>
      <aside className="sticky top-0 hidden h-screen w-[17rem] flex-col border-r border-slate-800/80 bg-slate-900/80 p-5 backdrop-blur-xl lg:flex">
        <div className="rounded-2xl border border-teal-300/20 bg-gradient-to-br from-teal-500/20 to-sky-500/10 p-4">
          ...
          <h2 className="mt-2 font-display text-lg text-white">ADMIN PORTAL</h2>
        </div>

        <nav className="mt-6 min-h-0 flex-1 overflow-y-auto pr-1">
          <div>
            <p className="px-2 text-[0.6875rem] uppercase tracking-[0.16em] text-slate-400">MANAGEMENT</p>
            <div className="mt-2 flex flex-col gap-1.5">
              {items.map((item, idx) => (
                <div key={item.path}>
                  {idx === items.length - 1 ? <div className="my-1 border-t border-slate-700/60" /> : null}
                  <Link to={item.path} className={`${navClass(item.path)} flex items-center gap-2 whitespace-nowrap pl-3 pr-2 text-sm`}>
