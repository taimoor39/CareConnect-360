function SystemHealth({ data }) {
  ...
  const normalizeServiceName = (service) => {
    ...
    if (/^ai\b|summar/i.test(value)) return 'AI Service';
    if (/database|mongo/i.test(value)) return 'Database';
    if (/email|smtp/i.test(value)) return 'Email Service';
    if (/cron|job/i.test(value)) return 'Scheduled Jobs';
    return value;
  };
      ...
      <div className="space-y-2 text-xs">
        {checks.map((check) => (
          <div key={check.service} className="rounded-lg bg-slate-900/70 px-3 py-2">
            <div className="flex items-center justify-between"><span className="text-slate-200">{normalizeServiceName(check.service)}</span><span className={check.status === 'Online' || check.status === 'Configured' || check.status === 'Active' || check.status === 'Connected' ? 'text-emerald-300' : check.status === 'Slow' ? 'text-amber-300' : 'text-rose-300'}>{check.status}</span></div>
